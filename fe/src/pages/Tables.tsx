...
function printTable(t: Table, dataUrl: string) {
  ...
  if (!w) return;
  w.document
    .write(`<!doctype html><html><head><meta charset="utf-8"><title>QR ${t.table_number}</title>
...
export default function Tables() {
  ...
  const rows = (data?.data ?? []).filter((t) =>
    t.table_number.toLowerCase().includes(q.toLowerCase()),
  );

  function renderActions(t: Table) {
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={async () => {
              try {
                await put<Table>(`/admin/tables/${t.id}`, {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Nomor meja dan kapasitas untuk cabang yang sedang dipilih.
            </DialogDescription>
          </DialogHeader>
          <form
            id="table-form"
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editTarget ? `Ubah meja ${editTarget.table_number}.` : ""}
            </DialogDescription>
          </DialogHeader>
          <form
            id="edit-table-form"
      ...
      >
        <DialogContent>
          ...
          <div className="flex flex-col items-center gap-4">
            {qrTarget && (
              <QrImage value={qrTarget.qr_link} size={240} onData={setQrUrl} />
            )}
          </div>
          <DialogFooter>
            <Button
