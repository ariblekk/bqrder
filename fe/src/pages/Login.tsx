...
export default function Login() {
  ...
  async function submitSetup(e: FormEvent) {
    ...
    try {
      ...
      setErr(ex instanceof ApiError ? ex.message : 'Gagal membuat akun, cek koneksi ke server')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setErr(ex instanceof ApiError ? ex.message : 'Gagal login, cek koneksi ke server')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="flex min-h-svh items-center justify-center p-4">
      <Card className="w-full max-w-sm">
        <CardHeader>
          <CardTitle className="text-xl">{bootstrap ? 'Setup Awal' : 'bqrder'}</CardTitle>
        </CardHeader>
        <CardContent>
          {bootstrap === null ? null : bootstrap ? (
            <form className="flex flex-col gap-4" onSubmit={submitSetup}>
              ...
              <p className="text-sm text-muted-foreground">
                Belum ada akun. Akun pertama otomatis menjadi Super Admin, beserta cabang pertama.
              </p>
