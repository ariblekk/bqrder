...
export const BASE = import.meta.env.VITE_API_URL || 'http://localhost:8080/api/v1'
const AUTH_KEY = 'bqrder_auth'
const BRANCH_KEY = 'bqrder_branch'
...
export const post = <T,>(path: string, data?: unknown) =>
  api<Envelope<T>>(path, { method: 'POST', body: data != null ? JSON.stringify(data) : undefined })
export const put = <T,>(path: string, data?: unknown) =>
  api<Envelope<T>>(path, { method: 'PUT', body: data != null ? JSON.stringify(data) : undefined })
export const del = <T,>(path: string) => api<Envelope<T>>(path, { method: 'DELETE' })
...
export async function upload<T = unknown>(path: string, file: Blob): Promise<T> {
  ...
  const body = await res.json().catch(() => null)
  if (!res.ok) throw new ApiError(res.status, body?.message || res.statusText || 'Upload failed')
  return body as T
}